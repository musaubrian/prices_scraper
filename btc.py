...
def convert_to_kes(usd: int):
    ...
    Args::
        usd(int): btc price in USD
    """
    convertion_rate = int(input("Enter current conversion rate(1~1000): "))
    curr_prefix = input("Currency prefix(e.g USD): ")
    usd_to_curr = usd * convertion_rate

    usd_shorthand = usd / 1000
    curr_shorthand = usd_to_curr / 1000000

    print(f"\nUSD {usd}\n 1 BTC = USD {usd_shorthand:.2f}K")
    print(f"{curr_prefix} {usd_to_curr}\n 1 BTC = {curr_prefix} {curr_shorthand:.2f}M")
    print("*"*25)
